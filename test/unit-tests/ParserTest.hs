...
parserExpressions = testGroup "parsing expressions"
  ...
  , testP "x && y()" (Binary [] AndOp (PathExpr [] Nothing (Path False [("x", NoParameters ())] ()) ())
                                      (Call [] (PathExpr [] Nothing (Path False [("y", NoParameters ())] ()) ()) [] ()) ())
  , testP "x || 1" (Binary [] OrOp (PathExpr [] Nothing (Path False [(mkIdent "x", NoParameters ())] ()) ()) (Lit [] (Int Dec 1 Unsuffixed ()) ()) ())
  , testP "x || y()" (Binary [] OrOp (PathExpr [] Nothing (Path False [("x", NoParameters ())] ()) ())
                                     (Call [] (PathExpr [] Nothing (Path False [("y", NoParameters ())] ()) ()) [] ()) ())
...
parserItems = testGroup "parsing items"
  ...
  , testP "mod foo { pub fn foo(x: i32) -> i32 { return x + 1 } }" (Mod [] InheritedV "foo" (Just [Fn [] PublicV "foo" (FnDecl [Arg (Just x) i32 ()] (Just i32) False ()) Normal NotConst Rust (Generics [] [] (WhereClause [] ()) ()) (Block [NoSemi (Ret [] (Just (Binary [] AddOp (PathExpr [] Nothing (Path False [(mkIdent "x", NoParameters ())] ()) ()) (Lit [] (Int Dec 1 Unsuffixed ()) ()) ())) ()) ()] Normal ()) ()]) ())
